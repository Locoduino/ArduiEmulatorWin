...
public:
    ...
    // scan result
    bool getNetworkInfo(uint8_t networkItem, String &ssid, uint8_t &encryptionType, int32_t &RSSI, uint8_t* &BSSID, int32_t &channel){return false;}

    String SSID(uint8_t networkItem){return NULL;}
    //wifi_auth_mode_t encryptionType(uint8_t networkItem);
    int32_t RSSI(uint8_t networkItem){return 0;}
    uint8_t * BSSID(uint8_t networkItem){return 0;}
    String BSSIDstr(uint8_t networkItem){return NULL;}
    int32_t channel(uint8_t networkItem){return 0;}
...
protected:
    ...
    static void * _getScanInfoByIndex(int i){return NULL;}
